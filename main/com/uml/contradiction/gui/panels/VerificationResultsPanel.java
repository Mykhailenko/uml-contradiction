...
//	private Map<Criterion, List<ResultTemplate> > resultsDescr = new  HashMap<Criterion, List<ResultTemplate> >();
	...
	public void showDescription(List<String> descriptions) {
    	...
    	for(int i = 0; i < descriptions.size(); i++) {
			...
			String[] parts = descriptions.get(i).split(ResultTemplate.ELEMENT_MARKER);
		...
		try {
			...
			for(int i = 0; i < descriptions.size(); i++) {
				...
				doc.setCharacterAttributes(curIndex, headerLen, heading2Style, false);
				curIndex = curIndex + headerLen;
				
				String[] descrParts = descriptions.get(i).split(ResultTemplate.ELEMENT_MARKER);
				System.out.println(descrParts);
				boolean isElement = false;
				for(int j = 0; j < descrParts.length; j++) {
					int partLen = descrParts[j].length();
					if(isElement) {
						doc.setCharacterAttributes(curIndex, partLen, elementStyle, false);
						curIndex = curIndex + partLen;
						isElement = false;
					}
					else {
						doc.setCharacterAttributes(curIndex, partLen, mainStyle, false);
						curIndex = curIndex + partLen;
						isElement = true;
					}
				}
	...
	public void setStyles() {
		this.mainStyle = sc.addStyle("MainStyle", sc.getStyle(StyleContext.DEFAULT_STYLE));
