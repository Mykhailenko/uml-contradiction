...
public class TransitionBelongToClass implements Predicate {
	...
	private boolean transitionBelongToMethod(Transition transition, MMethod method){
		if(transition.getShortName().equals(method.getName()) && 
				TransitionBelongToClass.paramsCount(transition.getName())  == method.getParameters().size()){
			return true;
		}else{
			return false;
		}
