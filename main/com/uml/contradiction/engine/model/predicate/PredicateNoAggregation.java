...
public class PredicateNoAggregation implements Predicate {
	...
	public boolean predict(List params) throws PredicatException {
		...
		if(params.size() != 1){
			...
			throw new PredicatException("PredicateIsEqualName require by list with 1 element");
		}
	
		if((params.get(0) instanceof com.uml.contradiction.model.cclass.AssociationEnd) == false){
			LOGGER.error("first element must be an AssociationEnd " + params.get(0).getClass().toString());
