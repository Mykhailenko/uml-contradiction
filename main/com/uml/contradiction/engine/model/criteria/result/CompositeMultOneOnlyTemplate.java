...
public class CompositeMultOneOnlyTemplate   extends ResultTemplate{
	...
	public void fill(HistoryPlainItem hpi) {
		...
		s = "A composition between classes " + 
		ResultTemplate.ELEMENT_MARKER+ cls.getName()+ResultTemplate.ELEMENT_MARKER + 
		" and " + ResultTemplate.ELEMENT_MARKER+cle.getName()+ResultTemplate.ELEMENT_MARKER + 
		...
		if(as.getEnd1().getAggregationKind()!=null && 
			...
			ResultTemplate.ELEMENT_MARKER+ cls.getName()+ResultTemplate.ELEMENT_MARKER + 
			" has multiplicity " + 
			ResultTemplate.ELEMENT_MARKER+"["+as.getEnd1().getMultiplicity().getLowerBound()+".."+
			as.getEnd1().getMultiplicity().getUpperBound()+"]"+ResultTemplate.ELEMENT_MARKER +". "; 
		}
		...
		if(as.getEnd2().getAggregationKind()!=null && 
			...
			s = s + "Composite end corresponding to class " + 
			ResultTemplate.ELEMENT_MARKER+ cle.getName()+ResultTemplate.ELEMENT_MARKER + 
			" has multiplicity " + 
			ResultTemplate.ELEMENT_MARKER+"["+as.getEnd2().getMultiplicity().getLowerBound()+".."+
			as.getEnd2().getMultiplicity().getUpperBound()+"]"+ResultTemplate.ELEMENT_MARKER +". "; 
		}
